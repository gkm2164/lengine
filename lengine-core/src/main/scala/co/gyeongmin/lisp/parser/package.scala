...
package object parser {
  private val ForbiddenOverrides: mutable.Set[String] =
    mutable.Set("fn", "def", "let", "do", "loop", "for", "in")
  ...
  private def parseTokenList: LispTokenState[LispList] = tokens => {
    @tailrec
    def loop(
      acc: Vector[LispToken],
      xs: LazyList[LispToken],
      parCnt: Int
    ): (List[LispToken], LazyList[LispToken]) =
      xs match {
    ...
    val result = token match {
      ...
      case (t @ LeftForcePar()) #:: afterLeftPar =>
        parseClause(afterLeftPar)
          .map(
            _.mapValue(_.wrapLocation(t.tokenLocation))
              .mapValue { case LispClause(body) =>
                LispClause(VarSymbol("force") :: body)
              }
          )
  ...
  private def parseDef: LispTokenState[LispValueDef] = {
    ...
    case VarSymbol(name) #:: _ if ForbiddenOverrides.contains(name) =>
      Left(DeniedKeywordError(name, "parseDef"))
  ...
  private def many[A](parser: LispTokenState[A]): LispTokenState[List[A]] = tks => {
    def loop(acc: Vector[A]): LispTokenState[List[A]] = {
      case LazyList()         => LispTokenState(acc.toList)(LazyList())
  ...
  private def option[A <: LispToken](parser: LispTokenState[A]): LispTokenState[Option[A]] =
    many(parser).flatMap {
