...
trait LispValue extends LispToken {
  ...
  def toInt: Either[EvalError, IntegerNumber] = Left(
    UnimplementedOperationError("toInt", this)
  )

  def toRatio: Either[EvalError, RatioNumber] = Left(
    UnimplementedOperationError("toRatio", this)
  )

  def toFloat: Either[EvalError, FloatNumber] = Left(
    UnimplementedOperationError("toFloat", this)
  )

  def toComplexNumber: Either[EvalError, ComplexNumber] = Left(
    UnimplementedOperationError("toComplexNumber", this)
  )
...
case class LispClause(body: List[LispValue]) extends LispValue {
  override def recoverStmt(): String =
    s"(${body.map(_.recoverStmt()).mkString(" ")})"
}
...
abstract class LispSeq extends LispValue {
  ...
  def toList: Either[EvalError, LispList] = Left(
    UnimplementedOperationError(s"$this is not a list value", this)
  )
}
...
case class LispList(items: List[LispValue]) extends LispSeq {
  ...
  override def recoverStmt(): String =
    s"(list ${items.map(_.recoverStmt()).mkString(" ")})"
}
...
case class LispForStmt(symbol: LispSymbol, seq: LispValue) extends LispValue {
  override def recoverStmt(): String =
    s"for ${symbol.name} in ${seq.recoverStmt()}"
}
...
case class LispValueDef(symbol: LispSymbol, value: LispValue)
    extends LispValue {
  override def recoverStmt(): String =
...
case class LispDoStmt(body: List[LispValue]) extends LispValue {
  override def recoverStmt(): String =
    s"(do ${body.map(_.recoverStmt()).mkString(" ")})"
}
...
case class LispFuncDef(symbol: LispSymbol, fn: GeneralLispFunc)
    extends LispValue {
  override def recoverStmt(): String =
    s"(fn ${symbol.recoverStmt()} (${fn.placeHolders
      .map(_.recoverStmt())
      .mkString(" ")}) ${fn.body.recoverStmt()})"
}
...
case class LispObject(kv: Map[ObjectReferSymbol, LispValue]) extends LispValue {
  ...
  private def keyValueString = kv
    .map { case (key, value) => s"${key.recoverStmt()} ${value.recoverStmt()}" }
    .mkString(" ")
