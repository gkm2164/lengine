...
sealed trait LispFunc extends LispValue {
  ...
  def placeHoldersAsString: String = if (placeHolders.nonEmpty)
    placeHolders.map(_.recoverStmt()).mkString(", ")
  else "no parameters"
...
abstract class BuiltinLispFunc(
  ...
  override def recoverStmt(): String =
    s"(lambda ${placeHolders.map(_.recoverStmt()).mkString(" ")} #native)"
}
...
case class LispMacro(
  ...
  override def recoverStmt(): String =
    s"(defmacro (${placeHolders.map(_.recoverStmt()).mkString(" ")}) ${body.recoverStmt()})"
}

case class GeneralLispFunc(placeHolders: List[LispValue], body: LispValue)
    extends LispFunc {
  override def recoverStmt(): String =
    s"(lambda (${placeHolders.map(_.recoverStmt()).mkString(" ")}) ${body.recoverStmt()})"
}
