...
class LispMacroReplace(clause: LispClause)(implicit runtimeEnvironment: LengineRuntimeEnvironment) {
  ...
  private def getMacroEnv(
    placeholders: List[LispValue],
    body: List[LispValue]
  ): Map[LispSymbol, LispValue] = {
    @tailrec
    def loop(
      acc: Map[LispSymbol, LispValue],
      ph: List[LispSymbol],
      body: List[LispValue]
    ): Map[LispSymbol, LispValue] =
      ph match {
  ...
  private class Replacer(implicit macroEnv: Map[LispSymbol, LispValue]) {
    def replaceLispValue: MacroReplacer[LispValue] = {
      ...
      case v =>
        throw CompileException(
          s"Unsupported replacement of macro: $v",
          runtimeEnvironment.fileName,
          clause.tokenLocation
        )
    }

    private def replaceLetDecl: MacroReplacer[LispLetDecl] = { case LispLetDecl(name, value) =>
      LispLetDecl(replaceSymbol(name).asInstanceOf[LispSymbol], replaceLispValue(value))
    }

    private def replaceLetDef: MacroReplacer[LispLetDef] = { case LispLetDef(decls, body) =>
      LispLetDef(decls.map(replaceLetDecl), replaceLispValue(body))
    }
    ...
    private def repalceForWhenStmt: MacroReplacer[LispForWhenStmt] = {
      case LispForWhenStmt(value, whenStmt, otherwise) =>
        LispForWhenStmt(
          replaceLispValue(value),
          whenStmt.map(replaceLispWhen),
          replaceLispValue(otherwise)
        )
    }

    private def replaceLispWhen: MacroReplacer[LispWhenStmt] = {
      case LispWhenStmt(value, thenValue) =>
        LispWhenStmt(replaceLispValue(value), replaceLispValue(thenValue))
    }
    ...
    private def replaceLispClause: MacroReplacer[LispClause] = { case LispClause(body) =>
      LispClause(body.map(replaceLispValue))
    }
    ...
    private def replaceDoStmt: MacroReplacer[LispDoStmt] = { case LispDoStmt(body) =>
      LispDoStmt(body.map(replaceLispValue))
    }

    private def replaceList: MacroReplacer[LispList] = { case LispList(items) =>
      LispList(items.map(replaceLispValue))
    }

    private def replaceFuncDef: MacroReplacer[LispFuncDef] = { case LispFuncDef(symbol, fn) =>
      LispFuncDef(replaceSymbol(symbol).asInstanceOf[LispSymbol], replaceLambda(fn))
    }
